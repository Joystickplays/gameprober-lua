...
local function main()
	...
	Explorer.StartDrag = function(offX,offY)
		...
		releaseEvent = input.InputEnded:Connect(function(input)
			if ((input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputService == Enum.UserInputType.Touch)) then
...
local function main()
	...
	Lib.ButtonAnim = function(button,data)
		...
		button.InputBegan:Connect(function(input)
			...
			if ((input.UserInputType == Enum.UserInputType.MouseMovement) or (input.UserInputType == Enum.UserInputType.Touch)) and not holding then
				if mode == 1 then
					button.BackgroundTransparency = 0.4
				elseif mode == 2 then
					button.BackgroundColor3 = control.HoverColor
				end
			elseif ((input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputService == Enum.UserInputType.Touch)) then
				holding = true
				if mode == 1 then
	...
	Lib.Window = (function()
		...
		local createGui = function(self)
			...
			guiTopBar.InputBegan:Connect(function(input)
				if ((input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputService == Enum.UserInputType.Touch)) and self.Draggable then
					local releaseEvent,mouseEvent
			...
			guiMain.InputBegan:Connect(function(input)
				if ((input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputService == Enum.UserInputType.Touch)) and not self.Aligned and not self.Closed then
					moveToTop(self)
				end
			end)
				...
				if item.Divider then
					...
					if not item.Disabled then
						...
						if item.OnRightClick then
                            ...
                            -- Handle touch or mouse hold for 0.5 seconds
                            newEntry.InputBegan:Connect(function(input)
                                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                                    ...
                                    local connection
                                    connection = newEntry.InputEnded:Connect(function(endInput)
                                    ...
                                    task.delay(holdTime, function()
                                        if held and (tick() - startTime) >= holdTime then
                                            item.OnRightClick(item.Name)
                                            if not item.NoHide then
                                                self:Hide()
                                            end
	...
	Lib.BrickColorPicker = (function()
		...
		local function hexInput(self,hex,color)
			hex.InputBegan:Connect(function(input)
				if ((input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputService == Enum.UserInputType.Touch)) and isMouseInHexagon(hex) then
			...
			hex.InputChanged:Connect(function(input)
				if ((input.UserInputType == Enum.UserInputType.MouseMovement) or (input.UserInputType == Enum.UserInputType.Touch)) and isMouseInHexagon(hex) then
		...
		local function new() -- TODO: Convert to newer class model
			...
			local function placePoint(point)
				...
				newSelect.InputBegan:Connect(function(input)
					...
					if ((input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputService == Enum.UserInputType.Touch)) and not currentlySelected then
						currentPoint = point
						local mouseEvent,releaseEvent
			...
			numberLine.InputBegan:Connect(function(input)
				if ((input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputService == Enum.UserInputType.Touch)) and #points < 20 then
	...
	Lib.ColorSequenceEditor = (function() -- TODO: Convert to newer class model
		local function new()
			...
			colorLine.InputBegan:Connect(function(input)
				if ((input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputService == Enum.UserInputType.Touch)) and #colors < 20 then
					local maxSize = colorLine.AbsoluteSize.X-1
					local relativeX = mouse.X - colorLine.AbsolutePosition.X
					if relativeX < 0 then relativeX = 0 end
					if relativeX > maxSize then relativeX = maxSize end
	...
	Main.SetMainGuiOpen = function(val)
		...
		if not val then
			...
			Main.MainGuiMouseEvent = service.UserInputService.InputBegan:Connect(function(input)
				if ((input.UserInputType == Enum.UserInputType.MouseButton1) or (input.UserInputService == Enum.UserInputType.Touch)) and not Lib.CheckMouseInGui(Main.MainGui.OpenButton) and not Lib.CheckMouseInGui(Main.MainGui.OpenButton.MainFrame) then
